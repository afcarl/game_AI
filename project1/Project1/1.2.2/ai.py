...
def learn_tree(node, turn):
    ...
    for i in index:
        ...
        if turn == 1: #Player X
            ...
            if score >= node.score or node.best_move == None:
                node.score = score
                node.best_move = i
...
def build_tree_and_learn():
    ...
    print "Number of nodes in tree = ", num_nodes_in_tree
    keys = DICTIONARY.keys()
    for key in keys:
        if DICTIONARY[key].best_move == None:
            DICTIONARY.pop(key)
    print "Dictionary after removing unnecassary entries: ", len(DICTIONARY)
...
def make_best_move(current_board):
    ...
    if best_move != None:
        ...
        print "Problem! Board not seen before! Playing at random"
        return [None, None]
