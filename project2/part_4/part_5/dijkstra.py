...
This class implements Dijstra Algorithm
"""
...
def dijkstra(graph, source):
    '''
    This function uses Dijstras Algorithm to find the shortest path between source and all
    ...
    return: (array of ints) Indicates the previous node to move to from a certain index (node)
    '''
    G = graph
    nodes = G.nodes()
    ...
    while len(Q) > 0:
        ...
        posibilities = np.where(distances==min_node_value)[0]#Get all posibilities to move to
        
        #Now choose one
        #Always go in order of down, right, left, up
        posibilities = np.sort(posibilities)[::-1] #So that path is same as in project desciption
        min_node_key = None
...
def get_shortest_path(graph, source, destination):
    '''
    This function is called when the shortest path is required using dijsktras
