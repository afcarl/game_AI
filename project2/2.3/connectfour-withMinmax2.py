...
Exercise 2.3: MinMax for connect4
'''
...
class Minmax(object):
    ...
    def __init__(self):
        ...
        #load a DIC with some precalculated values
        try: 
            self.DIC = self.load_obj('scores')
        except (RuntimeError, TypeError, NameError, OSError, IOError, EOFError):
            self.DIC  = {}
    ...
    def score(self, state, depth, cur_player):
        ...
        # else calculate
            ...
            # counting the number of good four/threes/twos in a row/column/diagonal...
    ...
    def min_play(self, board, depth, alpha ,beta , cur_player):
        ...
        minimizing the score, moving the opponent
        ...
        for eachmove in moves:
            # copy board and move opponent
            boardcopy = board.copy()
            
            board_i, _ , _ = move(boardcopy, oponent, eachmove)
            
            # build recursively max
...
def move_still_possible(S):
    ...
    returns True if grid contains no 0, therefore no move possible any more
    '''
...
def move_was_winning_move(S, p, num):
    ...
    @param S: 6x7 grid containing the current game state
    @param p: current player
    @param num: how many occurrences to count
